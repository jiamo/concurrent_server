...
class Select(BasePoller):
    ...
    def poll(self, timeout=None, max_events=None):
        ...
        rlist, wlist, xlist = select.select(list(self._rlist),
                                            list(self._wlist),
                                            list(self._xlist), timeout)
        for read in rlist:
            ret.append((read, select.POLLIN))
        for write in wlist:
            ret.append((write, select.POLLOUT))
        for err in xlist:
            ret.append((err, select.POLLERR))
        return ret
...
class Kqueue(BasePoller):
    ...
    def poll(self, timeout=None, max_events=100):
        """
        We have to call kqueue.control() here and modify the results to
        look like the results of the other pollers
        """
        ret = []
        for ev in self._poll.control(
                self._kev_table.values(), max_events,
                timeout):
            sock = self._sock_map[ev.ident]
            if sock.fileno() == -1:
                del self._kev_table[ev.ident]
                del self._sock_map[ev.ident]
                continue
            ret.append(
                (sock, self._rev_event_map[ev.filter]))
        return ret
